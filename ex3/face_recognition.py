...
def create_database(directory, show = True):
    ...
    Every image is cropped to the detected face, resized to 100x100 and save in another directory
    (orignal directory name + "2").
    ...
    for filename in fnmatch.filter(os.listdir(directory), '*.jpg'):
        ...
        result = cv2.resize(crop, (100, 100), interpolation=cv2.INTER_AREA)
...
def pca(X, nb_components=0):
    ...
    # Get the eigenvalues and eigenvectors of the covarianve matrix
    w, v = np.linalg.eig(cov)
    # Get the index of the eigenvalues in decreasing order
    idx = np.argsort(w)[::-1]
    # Order the eigenavalues and eigenvectors in decreasing order
    v[:] = v[idx]
    w[:] = w[idx]

    u = np.zeros((6, 10000))
    for i in range(n):
        u[i] = np.dot(newX.T, v[i])
        u[i] = normalize_vector(u[i])
    
    return w[:nb_components], u[:nb_components].T, mean
...
if __name__ == '__main__':
    ...
    create_dir("data/arnold2")
    create_dir("data/barack2")
